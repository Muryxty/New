...
for xd in range(10000):
	...
	c=random.choice(['zh-CN; MZ-MEIZU 18 Build/MRA58K'])
	d='AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/73.0.3683.121MZBrowser/10.2.2'
...
# gh = 'github.com/Al-Vino'
	cetak(nel('\tSelamat Datang [yellow]%s[white] Anak Haram👿'%(my_name)))
	...
	for user in uid:
		try:
			...
			if len(id) == 0:
				...
				params = ({'access_token': token,'fields': "friends"})
			col = ses.get('https://graph.facebook.com/{}'.format(user),params=params,headers=head,cookies={'cookies':cok}).json()
			...
			exit()
	try:
		print(f'ID Terkumpul : '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
